...
const proxyCache = lruCache({
  max: 1024,
  dispose: (key, item) => item.close(),
});

// Local web proxy.
exports.webProxy = function (request, response, head, parameters) {
  // Proxy request to the local port and path.
  const cacheKey = JSON.stringify(parameters.port);
  ...
  if (response instanceof http.ServerResponse) {
    ...
    proxy.ws(request, response, head);
  }
};
