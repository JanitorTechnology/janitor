...
// Get the cookiename saved from a previous session if any, or set a random one.
const cookieInfo = db.get('cookieInfo');
let cookieName;
if (!cookieInfo['name']) {
  // Generate a random cookie name made of letters 'a' to 'z' and digits.
  cookieName = Date.now().toString(36).replace(/[^a-z0-9]+/g, 'a');
  // Set the value in the db by providing it as default value to the `get` helper.
  db.set('cookieInfo', {name: cookieName});
} else {
