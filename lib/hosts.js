...
exports.authenticate = function (request) {
  let { clientId = null, clientSecret = null } = request.query;
...
exports.issueOAuth2AuthorizationCode = function (request, callback) {
  let { clientId, scope, state, redirectUrl = null } = request.query;
  const hostname = exports.identify(clientId);
  if (!hostname) {
    callback(new Error('No such OAuth2 client ID: ' + clientId));
    return;
  }

  if (!redirectUrl) {
    redirectUrl = 'https://' + hostname + '/';
  } else if (!redirectUrl.startsWith('https://' + hostname + '/')) {
    callback(new Error('Invalid OAuth2 redirect URL: ' + redirectUrl));
    return;
  }
  ...
  const onCode = (error, data) => {
    ...
    callback(null, { code, redirectUrl });
  };

  oauth2provider.generateAuthorizationCode(clientId, grant, state, onCode);
};
...
exports.issueOAuth2AccessToken = function (request, callback) {
  const { clientId, code, state } = request.query;
  const hostname = exports.identify(clientId);
  if (!hostname) {
    callback(new Error('No such OAuth2 client ID: ' + clientId));
    return;
  ...
  oauth2provider.generateAccessToken(clientId, code, state, onToken);
};
