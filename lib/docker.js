...
// Load default Docker client certificates and host configuration.
...
function getDocker (host, callback) {
  ...
  var hosts = db.get('dockers');
  var configuration = hosts[host || 'localhost'];
  ...
  var docker = new dockerode({
    ...
    ca: configuration.ca || client.ca,
    cert: configuration.cert || configuration.crt || client.crt,
    key: configuration.key || client.key
  });
...
// Change an existing Docker host configuration, or add a new host.

function setHost (parameters) {

  var hosts = db.get('dockers');
  var configuration = hosts[parameters.id];

  if (!configuration) {
    configuration = {};
    hosts[parameters.id] = configuration;
  }
  ...
  // Apply the requested changes to the host configuration.
  jsonpatch.apply(configuration, operations);
  db.save();
