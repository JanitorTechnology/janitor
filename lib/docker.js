...
function getDocker (hostId, callback) {

  var host = hosts.get(hostId);

  if (!host) {
    return callback(new Error('Unknown Docker Host: ' + hostId));
  }

  var docker = new dockerode({
    protocol: 'https',
    host: host.properties.hostname,
...
// DEPRECATED. Change an existing Docker host configuration, or add a new host.
...
function setHost (parameters) {
  ...
  jsonpatch.apply(host.properties, operations);
  db.save();
