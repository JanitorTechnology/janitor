...
exports.getAvailableMachines = function (user) {
  let machines = {};
  for (let projectId in db.get('projects')) {
...
exports.getMachineByContainer = function (user, hostname, containerId) {
  ...
  for (let projectId in user.machines) {
    for (let machine of user.machines[projectId]) {
...
exports.setProject = function (parameters) {
  ...
  let project = getProject(parameters.id, true);
...
exports.rebuild = function (projectId, callback) {
  ...
  let dockerfile = project.docker.build;
  let time = Date.now();
  ...
  docker.buildImage(parameters, (error, stream) => {
    if (error) {
...
exports.update = function (projectId, callback) {
  ...
  let host = project.docker.host;
  let image = project.docker.image;
  let dockerfile = project.docker.update;
  let time = Date.now();
...
exports.spawn = function (user, projectId, callback) {
  ...
  docker.buildImage(parameters, function (error, stream) {
    ...
    stream.on('end', () => {
      ...
      for (let port in machine.docker.ports) {
...
exports.destroy = function (user, projectId, machineId, callback) {
  ...
  docker.removeContainer(parameters, (error) => {
    if (error) {
...
function getMachine (user, projectId) {
  ...
  for (let port in project.docker.ports) {
    if (!machine.docker.ports[port]) {
