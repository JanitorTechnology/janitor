...
exports.spawn = function (user, projectId, callback) {
  ...
  docker.runContainer({ host, image, ports }, (error, container, logs) => {
    ...
    resetSSHAuthorizedKeys(user, machine, error => {
      log('spawn-sshkeys', container.id.slice(0, 16), error || 'success');
      db.save();
    });

    // Install all the user's configuration files into this container.
    Object.keys(user.configurations).forEach(file => {
      exports.resetConfigurationFile(user, file, machine, error => {
        log('spawn-config', file, container.id.slice(0, 16), error || 'success');
      });
    });
...
exports.deployConfigurationFileInAllContainers = function (user, file) {
  const userMachines = Object.values(user.machines)
    .filter(m => m.status == 'started')
    .reduce((acc, val) => acc.concat(val), []);

  return Promise.all(userMachines.map(machine => {
    const { container } = machine.docker;
    if (!container || container.length < 16 || !/^[0-9a-f]+$/.test(container)) {
      log("[fail] Container is invalid", container);
      return;
    }

    return exports.deployConfigurationFile(user, file, machine);
  }));
};

exports.deployConfigurationFile = function (user, file, machine) {
...
function resetSSHAuthorizedKeys (user, machine, callback) {
