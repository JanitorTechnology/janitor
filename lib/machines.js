...
exports.spawn = function (user, projectId, callback) {
  ...
  docker.runContainer({ host, image, ports }, (error, container, logs) => {
    ...
    for (let file in user.configurations) {
      const f = file;
      resetConfigurationFile(user, file, machine, error => {
        log('spawn-config', f, container.id.slice(0, 16), error || 'success');
      });
    }
...
function resetConfigurationFile(user, file, machine, callback) {
  ...
  docker.copyIntoContainer({
    ...
    const command = `sudo chown user:user /home/user/.ssh/${file}`;
    docker.execInContainer({ host, container: containerId, command }, error => {
      callback(error);
    });
  });
}
exports.resetConfigurationFile = resetConfigurationFile;
