...
exports.getAvailableMachines = function (user) {
  ...
  for (const projectId in db.get('projects')) {
    machines[projectId] = getOrCreateMachine(user, projectId);
  }
...
exports.spawn = function (user, projectId, callback) {
  ...
  const machine = getOrCreateMachine(user, projectId);
  if (machine.status === 'started') {
    callback(null, machine.docker.logs);
    return;
  }
...
function getOrCreateMachine (user, projectId) {
  const project = getProject(projectId);
  if (!project) {
